...
export default ({ open, onClose }: Props) => {
    ...
    async function post() {
        ...
        try {
            ...
            const s3UploadInformation = await apitest.json<{ bucket: string; key: string; url: string; }>("/s3", { headers, params: `?capture_id=${capture_id}&capture_date=${capture_date.toString()}` }) || {};

            if (s3UploadInformation?.url) {
                await cloud.uploadScreenshot({ url: s3UploadInformation.url, data: screenshot });
            }

            await apitest.postJson(`/autorun?workstream=${autorun?.workstream?.workstream_id}&account_key=${autorun?.accountKey}`, { ...data, screenshot: { ...s3UploadInformation, url: s3UploadInformation?.url?.split("?")?.[0] }}, { headers });
    ...
    async function next() {
        ...
        try {
            ...
            const template = await apitest.json("/autorun", { headers, params });
            if (template?.url) {
                setStatus("Running");
                setId(template.id);
